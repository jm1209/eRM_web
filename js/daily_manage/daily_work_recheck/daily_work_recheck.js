layui.use(['form', 'layer', 'table', 'jquery', 'laydate'], function () {
    ...
    //数据表格渲染
    tableInit(table, 'routineBackend/getDailyWorkReviewList', [[
        ...
        {field: 'dealUser', title: '处理人', align: 'center'},
        {
            title: '操作', width: 100, align: 'center', templet: function (d) {
                if ((d.workStatus == "2" || d.workStatus == "1") && isReview) {
    ...
    //添加和编辑
    function addOrEdit(title, edit, url) {
    ...
    table.on('tool(tableList)', function (obj) {
        ...
        if (layEvent === 'handle') { //编辑
            addOrEdit('处理', data, 'recheck.html');
        } else if (layEvent === 'see') { //编辑
            addOrEdit('查看', data, 'recheck_see.html');
        }
    });
});
