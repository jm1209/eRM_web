layui.use(['form', 'layer', 'jquery', 'upload', 'element'], function () {
    ...
    setTimeout(function () {
        ajaxJS(param, {workId: $('.sign').attr('workId')}, function (d) {
            ...
            $('.visionRemark').val(d.data.visionRemark);
            $('.reviewRemark').val(d.data.reviewRemark);
    ...
    $('.put').click(function () {
        ...
        param.url = 'routineBackend/reviewDailyWork';
        ajaxJS(param, {workId: $('.sign').attr('workid'), reviewRemark: $('.reviewRemark').val()}, function (d) {
            layer.msg(d.desc);
    ...
    function insertLastTask(lastDailyArr) {
        for (var i = 0; i < lastDailyArr.length; i++) {
            var taskStr = '<div class="task-list" taskId="' + lastDailyArr[i].taskId + '">' +
                ...
                '             <span>缺陷等级:</span>' +
                '             <span class="bug">' +
                '             <select disabled class="dangerLevel" danger="' + lastDailyArr[i].dangerLevel + '">' +
                ...
                '             <span>是否通过: </span>' +
                '             <span class="">' +
                '             <select disabled class="isPass" onchange="isPass(this)">' +
                ...
                '             <div class="layui-input-block">' +
                '             <textarea disabled placeholder="请输入内容" class="layui-textarea sug">' + (lastDailyArr[i].approveRemark || "同意") + '</textarea>' +
                '             </div>' +
                '             </div>' +
                '              </div>' +
                '               </div>';
    ...
    function insertTask(dailyArr) {
        for (var i = 0; i < dailyArr.length; i++) {
            var taskStr = '<div class="task-list" taskId="' + dailyArr[i].taskId + '">' +
                ...
                '             <span>缺陷等级:</span>' +
                '             <span class="bug">' +
                '             <select disabled class="dangerLevel" danger="' + dailyArr[i].dangerLevel + '">' +
                '              <option value="0">关闭</option>' +
                '             </select>' +
                '             </select>' +
                '             </span>' +
                ...
                '             <label class="layui-form-label">审批意见</label>' +
                '             <div class="layui-input-block">' +
                '             <textarea disabled class="layui-textarea sug">' + (dailyArr[i].approveRemark || "同意") + '</textarea>' +
                '             </div>' +
                '             </div>' +
                '              </div>' +
                '               </div>';
