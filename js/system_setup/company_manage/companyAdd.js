layui.use(['form', 'layer', 'jquery', 'upload'], function () {
    ...
    if (sessionStorage.getItem('companyId') != 'null') {
        $('.yzCompany').append('<option value="' + sessionStorage.getItem('companyId') + '">' + sessionStorage.getItem('companyName') + '</option>').attr('disabled', true);
        form.render()
    }
    ...
    form.on("submit(addOrEdit)", function (data) {
        ...
        var data = {
            ...
            pid: field.yzCompany || $('.yzCompany option').attr('value'),
            logoUrl: $('.showImg img').attr('src')
        };

        if ($(".sign").val() == "edit") {  //编辑
            data.companyId = $(".sign").attr("signid");
            param.url = 'systemCenter/updateCompanyProject';
            ...
            param.url = 'systemCenter/addCompanyProject';
            ajaxJS(param, data, function (d) {
                top.layer.close(index);
                top.layer.msg(d.desc);
                layer.closeAll("iframe");
