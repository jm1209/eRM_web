...
layui.define(["element", 'layer', "jquery"], function (exports) {
    ...
    Tab.prototype.render = function () {
        ...
        ajaxJS(param, {empId: sessionStorage.getItem("empId")}, function (d) {
            //显示左侧菜单
            if ($(".navBar").html() == '') {
                var _this = this;
    ...
    var itemStatus = $("#main").contents().find(".pillar");
    $(itemStatus).each(function () {
        if ($(this).attr('projectstatus') == '6') {
            $('#main').attr('src', 'html/daily_manage/daily_pandect/daily_pandect.html');
        } else {
            $('#main').attr('src', 'html/project_setup/plan_pandect/plan_pandect.html');
        }
    });
