layui.use(['form', 'layer', 'table', 'jquery', 'laydate'], function () {
    ...
    //数据表格渲染
    tableInit(table, 'projectBackend/getApproveProjectList', [[
        ...
        },
        {
            title: '操作', minWidth: 150, align: "center", templet: function (d) {
                if ((d.projectStatus == "3" || d.projectStatus == "7") && isApprove) {
    ...
    table.on('tool(tableList)', function (obj) {
        ...
        if (layEvent === 'edit') { //编辑
            addOrEdit('项目计划审批', data);
        } else if (layEvent === 'see') {
            var index = layui.layer.open({
                title: '项目计划审批查看',
                type: 2,
                content: "approvalSee.html",
                success: function (layero, index) {
                    var body = layui.layer.getChildFrame('body', index);
